...
function bankerRound(number, decimalDigits) {
    ...
    var addedZeros = Math.pow(10, decimalDigits + 2);
    // remove the extra zero we added to deal with JavaScript's float issues
    var trunc = Math.round(number * addedZeros * 0.1);
    switch (trunc % 10) {
...
function calcBalance(balance, payment, interestRate, periodsPerYear, paymentNumber, accInterest, payTable) {
...
function loanCalc(principal, interestRate, periodsPerYear, numberOfYears) {
    ...
    var n = periodsPerYear * numberOfYears;
    var r = interestRate / periodsPerYear;
    var payment = (principal * r) / (1 - Math.pow(1 + r, -1 * n));
    ...
    var loanTotal = payment * periodsPerYear * numberOfYears;
    var paymentTable = calcBalance(principal, payment, interestRate, periodsPerYear, 1, 0, []);
    return {
...
try {
    if (process.argv !== undefined) {
        ...
        if (args.length !== 4) {
            ...
            try {
                args.map(function (elm, ind) {
                    var num = Number.parseFloat(elm);
                    if (isNaN(num)) {
                        throw "Argument " + (ind + 1) + ", \"" + elm + "\" can't be converted to a number.";
                    }
                    else {
                        args[ind] = num;
                    }
